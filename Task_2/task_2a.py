...
def control_logic(sim):
	...
	while 1:
		...
		distance2 = detect_distance_sensor_2(sim)			#Right distance
		distance3 = detect_distance_sensor_3(sim)			#Left distance
		...
		# sim.setJointTargetVelocity(rmotor,vel)
		# print("test")
		if distance1 == 0 and distance2<=0.19 and distance2>=0.16 and distance3>=0.19 and distance3<=0.22:		#Straight
			print("Straight")
			sim.setJointTargetVelocity(lmotor,vel)
			sim.setJointTargetVelocity(rmotor,vel)

		elif distance3 == 0 and distance1 <= 0.21 and distance1 != 0:			#Left turn encountered
			# while  distance2>0.19 and distance2<0.21 and distance1 != 0:
			for i in range(45):
				print("Left Turn")
				sim.setJointTargetVelocity(lmotor,-1)
				sim.setJointTargetVelocity(rmotor,1)
		
		elif distance2 == 0 and distance1 <= 0.21 and distance1 != 0:			#Right turn encountered
			# while  distance3>0.19 and distance3<0.21 and distance1 != 0:
			for i in range(45):
				print("Right turn")
				sim.setJointTargetVelocity(lmotor,1)
				sim.setJointTargetVelocity(rmotor,-1)

		elif (distance2<0.20) and distance3<0.23:
			sim.setJointTargetVelocity(lmotor,1)
			sim.setJointTargetVelocity(rmotor,1)
			print("none")

		
		# elif distance2 > 0.20 and distance3 < 0.172 and distance1 == 0:			#bot deviates left
		# 	print("tilt right")
		# 	sim.setJointTargetVelocity(lmotor,0.1)
		# 	sim.setJointTargetVelocity(rmotor,0)

		# elif distance2 < 0.20 and distance3 > 0.172 and distance1 == 0:			#bot deviates right
...
def detect_distance_sensor_2(sim):
	...
	distance = None
	##############  ADD YOUR CODE HERE  ##############
	right_proximity = sim.getObjectHandle('distance_sensor_2')
	flag,distance,detectedPoint,detectedObjectHandle,detectedSurfaceNormalVector=sim.readProximitySensor(right_proximity)
	# print(flag,distance)
...
def detect_distance_sensor_3(sim):
	...
	distance = None
	##############  ADD YOUR CODE HERE  ##############
	left_proximity = sim.getObjectHandle('distance_sensor_3')
	flag,distance,detectedPoint,detectedObjectHandle,detectedSurfaceNormalVector=sim.readProximitySensor(left_proximity)
	# print(flag,distance)
